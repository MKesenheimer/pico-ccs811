...
class CCS811 {
  public: // Main interface
    ...
    bool init();                                                       // Reset the CCS811, switch to app mode and check HW_ID. Returns false on problems.
  ...
  public: // Extra interface
    int  hardware_version();                                              // Gets version of the CCS811 hardware (returns -1 on I2C failure).
    int  bootloader_version();                                            // Gets version of the CCS811 bootloader (returns -1 on I2C failure).
    int  application_version();                                           // Gets version of the CCS811 application (returns -1 on I2C failure).
    int  get_errorid();                                                   // Gets the ERROR_ID [same as 'err' part of 'errstat' in 'read'] (returns -1 on I2C failure).
  ...
  public: // Advanced interface: i2cdelay
    void set_i2cdelay(uint16_t us);                                                // Delay before a repeated start - needed for e.g. ESP8266 because it doesn't handle I2C clock stretch correctly.
    int  get_i2cdelay();                                                  // Get current repeated start delay.
  protected: // Helper interface: nwake pin
    void wake_init();                                                     // Configure nwake pin for output. If nwake<0 (in constructor), then CCS811 nWAKE pin is assumed not connected to a pin of the host, so host will perform no action.
    void wake_up();                                                       // Wake up CCS811, i.e. pull nwake pin low.
    void wake_down();                                                     // CCS811 back to sleep, i.e. pull nwake pin high.
    bool abort_begin();
  protected: // Helper interface: i2c wrapper
    bool i2cwrite(uint8_t regaddr, size_t count, const uint8_t* buf);                // Writes `count` from `buf` to register at address `regaddr` in the CCS811. Returns false on I2C problems.
    bool i2cread (uint8_t regaddr, size_t count, uint8_t* buf);                      // Reads 'count` bytes from register at address `regaddr`, and stores them in `buf`. Returns false on I2C problems.
  private:
