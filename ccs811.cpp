...
#define PRINTLN2(s,m) std::cout << s << std::endl;
#define PRINT2(s,m)   std::cout << s;
//#define PRINTLN(s)
//#define PRINT(s)
//#define PRINTLN2(s,m)
//#define PRINT2(s,m)
...
bool CCS811::init() {
  ...
  wake_up();
    ...
    sleep_us(CCS811_WAIT_AFTER_RESET_US);
    ...
    sleep_us(CCS811_WAIT_AFTER_APPSTART_US);
...
bool CCS811::flash(const uint8_t * image, int size) {
  ...
  wake_up();
    ...
    sleep_ms(CCS811_WAIT_AFTER_APPERASE_MS);
    PRINTLN(F("ok"));
    ...
    while( size>0 ) {
        ...
        PRINT(F("."));
        sleep_ms(CCS811_WAIT_AFTER_APPDATA_MS);
        image+= len;
        size-= len;
        count++;
    ...
    sleep_ms(CCS811_WAIT_AFTER_APPVERIFY_MS);
    PRINTLN(F("ok"));
...
void CCS811::set_i2cdelay(uint16_t us) {
  _i2cdelay_us = us;
}


// Get current repeated start sleep_ms.
int  CCS811::get_i2cdelay() {
  return _i2cdelay_us;
}
...
void CCS811::wake_init() {
  if(_nwake >= 0)
    gpio_set_dir(_nwake, GPIO_OUT);
}
...
void CCS811::wake_up() {
  if(_nwake >= 0) { 
    gpio_put(_nwake, 0);
    sleep_us(CCS811_WAIT_AFTER_WAKE_US);
  }
}

// CCS811 back to sleep, i.e. pull nwake pin high.
void CCS811::wake_down() {
  if(_nwake >= 0) 
    gpio_put(_nwake, 1);
}
...
bool CCS811::i2cwrite(uint8_t regaddr, size_t count, const uint8_t* buf) {
  uint8_t buf2[count + 1];
  buf2[0] = regaddr;
  memcpy(&buf2[1], buf, count);
  size_t r = (size_t)i2c_write_blocking(i2c0, _slaveaddr, buf2, count + 1, false);
...
bool CCS811::i2cread(uint8_t regaddr, size_t count, uint8_t* buf) {
  uint8_t addr[] = {regaddr};
  i2c_write_blocking(i2c0, _slaveaddr, addr, 1, true);
  size_t r = (size_t)i2c_read_blocking(i2c0, _slaveaddr, buf, count, false);
  return r == count;
